...
public class SI{
	public static void main(String[] args){
		...
		System.out.println("初期ステータースガ決定しました");
		System.out.print("種族を選んでください0…人間,1...ハイエルフ,2...トロル,3...ノーム>");
		int race=sc.nextInt();
		System.out.println(races[race]+"ボーナスが適用されました！");
		...
		System.out.printf("私の%s、%sです、%n",classes[cls],races[race],name);
	...
	static void classBonus(int[] status,double[]raito){
		for(int i=0;i<status.length;i++){
			status[i]=(int)(status[i]*raito[i]);
		}
	}
